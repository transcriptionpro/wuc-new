...
When the number of transactions in the past 10 minutes exceeds this speed limit, a bidding war occurs. Bitcoin transactions are prioritized based on the transaction fee attached. Higher

￼fee paid by the user, the more likely their transaction will be prioritized before a lesser­paying user.
...
Conversely, a prevailing view is that this speed limit presents a severe constraint on growth and adoption. The speed limit, in the face of userbase growth, will lead to higher fees, ultimately reaching an equilibrium when the system capacity of 7 transactions/second is

￼reached. This will cause a ripple effect where blockchain­centered business plans are shelved, or never started in the first place, for lack of perceived scalability.
